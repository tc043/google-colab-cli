---
log:
2026-09-10: Ported PR #109 onto the Windows-support branch without regressing its platform behavior. Console raw-TTY setup now remains conditional on `termios`/`tty` availability, POSIX-only resize signals are guarded on Windows, and Windows piped stdin uses non-blocking file-descriptor polling instead of `select()` (which only accepts sockets there). Runtime-token refresh, reconnect recovery, and flow-control acknowledgement remain shared across platforms.
