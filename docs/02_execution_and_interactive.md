---
log:
2026-09-10: Ported PR #109 onto the Windows-support branch and verified it live against Colab. Interactive-console detection is now separate from POSIX raw-mode availability, so Windows TTY sessions retain reconnect/exit semantics even though `termios`/`tty` and `SIGWINCH` are unavailable. Windows pipe input uses non-blocking file-descriptor polling instead of `select()` (which only accepts sockets there), and the reconnect/flow-control integration harnesses use a cross-platform synthetic TTY on Windows. Live CPU regressions passed for runtime-token refresh (`ls`, `exec`, piped `console`), forced same-endpoint Console reconnect, concurrent local binding removal, and >600 KB PTY flow-control acknowledgement without disturbing pre-existing assignments.
...
- **Terminal Management**: On platforms with `termios`/`tty`, configures `sys.stdin` to raw mode, passes single characters to the socket, writes raw ANSI escape sequences directly to `sys.stdout.buffer`, and hooks `SIGWINCH` when available to communicate local terminal dimensions (`cols`/`rows`). Windows does not expose these POSIX primitives, so Console leaves terminal mode untouched there while retaining the same WebSocket recovery path.
...
- **Piped stdin**: Detected via `sys.stdin.isatty()`. When piped, the input characters are forwarded one at a time to the remote pty, and on EOF the client sends `exit\n` and then closes the websocket itself after `PIPED_EOF_GRACE_SECONDS` (0.5s) so the user's shell goodbye text drains back. POSIX uses `select()` to wait for readable stdin; Windows uses a temporary non-blocking file-descriptor read loop because WinSock `select()` only accepts sockets. The remote `/colab/tty` endpoint wraps bash in tmux, which intercepts a bare `\x04` as a literal character — that is why we send `exit\n` rather than Ctrl-D.
