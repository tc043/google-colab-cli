...
- **Kernel activity supplement**: Every fifth loop iteration, the daemon may execute a trivial cell to register kernel-level activity, but only when the session is not already marked `running`. This supplement is deliberately subordinate to the TFE heartbeat: it uses the runtime-proxy refresh/retry path, closes its client afterward, logs failures as non-terminal, and must never block or terminate future 60-second TFE pings. Long-running `exec`/`repl`/`console` calls therefore skip the synthetic cell instead of queueing behind user work.
