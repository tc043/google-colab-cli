---
log:
2026-09-12: Isolated synthetic kernel activity from the TFE heartbeat after a long-lived session showed a second failure mode: Jupyter websocket setup could hang before `execute_code(timeout=...)` took effect, freezing the keep-alive daemon and causing backend pruning about an hour later. Kernel activity now runs in a disposable one-shot child process with a 30-second hard watchdog plus parent-side stale-worker reap; the parent heartbeat loop only polls the worker and never waits for kernel/WebSocket I/O.
2026-09-12: Fixed a keep-alive regression that made active sessions disappear after ~66 minutes. The daemon's first synthetic kernel ping (~5 minutes after launch) imported the removed `is_terminal_error` helper and could crash silently; long-running user cells could also block that synthetic ping and therefore stop the authoritative 60-second TFE heartbeat. Kernel activity is now strictly best-effort: it is skipped while `SessionState.running` is set, uses runtime-proxy refresh when idle, and failures are logged without terminating or delaying the TFE loop.
...
- **Kernel activity supplement**: Every fifth loop iteration, the daemon may launch a disposable one-shot `kernel-ping` child process to execute a trivial cell and register kernel-level activity, but only when the session is not already marked `running`. The parent daemon never performs Jupyter/WebSocket work inline and never waits for the child; it only polls the child on later heartbeat cycles. The worker uses the runtime-proxy refresh/retry path, closes its client afterward, and carries a 30-second process watchdog because `execute_code(timeout=...)` does not bound websocket setup. The parent also kills any worker that remains alive past its stale-worker deadline. Long-running `exec`/`repl`/`console` calls skip the synthetic cell, and a hung/failed kernel worker cannot delay or terminate future TFE pings.
...
- **Test Case (kernel-ping isolation)**: Verify a kernel worker that never exits does not suppress later TFE heartbeat iterations, child-process flags preserve auth/config, and the one-shot worker arms its hard watchdog before touching Jupyter/WebSocket state.
