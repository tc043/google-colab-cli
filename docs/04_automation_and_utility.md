---
log:
2026-09-12: Hardened session history JSONL for concurrent Windows writers. Foreground CLI commands and detached keep-alive daemons now serialize per-session history appends with a cross-process file lock, preventing lost records and zero-filled gaps observed under concurrent append. History reads use the same lock, recover legacy records prefixed by NUL bytes, and skip an unrecoverable malformed record with a warning instead of crashing `colab log` or export.
