...
| `repro_console_reconnect/` | CPU-only live fault injection: abruptly drops an interactive Console WebSocket, verifies visible same-endpoint reconnection and remote command execution, then confirms pre-existing assignments are unchanged. |
